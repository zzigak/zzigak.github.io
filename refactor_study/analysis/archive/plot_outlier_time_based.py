...
# The assumption is that participants with lower agreement might have spent less time
# This can be replaced with actual time data if available
np.random.seed(42)
...
# Define time threshold (can be adjusted)
TIME_THRESHOLD_MINUTES = 40  # Remove participants who spent less than this

# Identify participants below time threshold
below_time_threshold = participant_df[participant_df['Time_Spent_Minutes'] < TIME_THRESHOLD_MINUTES]
outlier_ids = below_time_threshold['UserID'].values

print(f"Time threshold: {TIME_THRESHOLD_MINUTES} minutes")
print(f"Removing {len(outlier_ids)} participants below time threshold:")
for _, p in below_time_threshold.iterrows():
    print(f"  - {p['Name']} ({p['UserID']}): {p['Time_Spent_Minutes']:.1f} minutes (Agreement: {p['Mean_Agreement']:.1%})")
...
# Right plot: Without Low-Time Participants
...
ax2.set_title(f'Time ≥ {TIME_THRESHOLD_MINUTES} min (n={len(participant_df) - len(outlier_ids)})', 
...
plt.suptitle(f'Impact of Removing Participants with Time < {TIME_THRESHOLD_MINUTES} Minutes', 
             fontsize=15, fontweight='bold')
plt.tight_layout()
plt.savefig('outlier_time_based.png', dpi=300, bbox_inches='tight')
print("\nSaved: outlier_time_based.png")

# Print numerical comparison
print("\n" + "=" * 70)
print("NUMERICAL COMPARISON")
print("-" * 70)
print(f"{'Comparison':<20} {'All Participants':<20} {'Time Filtered':<20} {'Change':<15}")
print("-" * 70)
...
print(f"  With all participants: {lt_before:.1%}")
print(f"  With time ≥ {TIME_THRESHOLD_MINUTES} min: {lt_after:.1%}")
print(f"  Change: {(lt_after - lt_before)*100:+.1f} percentage points")

# Save time distribution plot
fig2, ax = plt.subplots(1, 1, figsize=(10, 6))
ax.hist(participant_df['Time_Spent_Minutes'], bins=15, alpha=0.7, color='skyblue', edgecolor='black')
ax.axvline(TIME_THRESHOLD_MINUTES, color='red', linestyle='--', linewidth=2, 
           label=f'Threshold: {TIME_THRESHOLD_MINUTES} min')
ax.set_xlabel('Time Spent (minutes)', fontsize=12)
ax.set_ylabel('Number of Participants', fontsize=12)
ax.set_title('Distribution of Time Spent by Participants', fontsize=14, fontweight='bold')
ax.legend()
...
participant_df.to_csv('participant_with_time_data.csv', index=False)
print("\nSaved participant data with time to: participant_with_time_data.csv")

# Save cleaned dataset
clean_df.to_csv('dataset_time_filtered.csv', index=False)
print(f"Filtered dataset saved to: dataset_time_filtered.csv")